...
let spacePressed = false; // Variável para controlar se a tecla de espaço foi pressionada
...
document.addEventListener('keyup', (e) => {
    if (e.keyCode === 32 && spacePressed) { // Se a tecla de espaço foi pressionada
        const pauseButton = document.getElementById('btnpause');
        if (gameOver) return; // Se o jogo já estiver encerrado, não faz nada
        if (paused) {
            // Se o jogo estiver pausado, retome o jogo
    ...
    spacePressed = false; // Reseta a variável para a próxima vez que a tecla de espaço for pressionada
});
